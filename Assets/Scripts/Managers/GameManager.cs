...
public class GameManager : MonoBehaviour
{
    ...
    public enum Missions { Home = 1, Mission1, Mission2, Mission3, Mission3Boss }

    float totalGameTime;                        //Length of the total game time
    public bool isGameOver { get; set; }                            //Is the game currently over?
    int score = 0;
    int initialBombs = 10;
    int bombs;
        ...
        get
        {
            if (current == null)
            {
                ...
                if (current == null)
                {
                    ...
                    current = singletonObject.AddComponent<GameManager>();
    ...
    public static void PlayerDied()
    {
        ...
        string player = PlayerPrefs.GetString(ProfileManager.Instance.playerName);

        if (current.currentMission == Missions.Mission1 && current.score > GetMission1Points())
        {
            SetMission1Points(current.score);
            SaveManager.SetRecords(new Records(player, current.score, GetMission2Points(), GetMission3Points()));
        }
        else if (current.currentMission == Missions.Mission2 && current.score > GetMission2Points())
        {
            SetMission2Points(current.score);
            SaveManager.SetRecords(new Records(player, GetMission1Points(), current.score, GetMission3Points()));
        }
        else if (current.currentMission == Missions.Mission3Boss && current.score > GetMission3Points())
        {
            SetMission3Points(current.score);
            SaveManager.SetRecords(new Records(player, GetMission1Points(), GetMission2Points(), current.score));
        }
    ...
    public static GameObject GetPlayer(GameObject player)
    {
        if (current == null)
            return null;
        if (player.GetComponent<JackController>()) // return itself
            return player;
        else if (player.transform.parent.gameObject.GetComponent<JackController>()) // return parent
